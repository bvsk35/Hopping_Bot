...
class CartPole_GPS(object):
    ...
    def augment_state(self, X):
        if X.ndim == 1:
            ...
            u_x = X[..., 4].reshape(-1, 1)
        return np.hstack([x, x_dot, np.sin(theta), np.cos(theta), theta_dot, u_x])
    ...
    def deaugment_state(self, X):
        if X.ndim == 1:
            x, x_dot, sin_theta, cos_theta, theta_dot, u_x = X
        else: 
            x = X[..., 0].reshape(-1, 1)
            x_dot = X[..., 1].reshape(-1, 1)
        ...
        return np.hstack([x, x_dot, theta, theta_dot, u_x])
    ...
    '''
    def accel(self, X, Xd):
        temp = (X[5] + Xd[0] * Xd[2] * X[4]**2 * X[2])/(Xd[1] + Xd[0])
