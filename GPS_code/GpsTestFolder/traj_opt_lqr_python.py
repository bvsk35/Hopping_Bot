...
class TrajOptLQRPython(TrajOpt):
    ...
    def backward(self, prev_traj_distr, traj_info, eta, algorithm, m):
        ...
        if self._update_in_bwd_pass:
            ...
            #  this function is make all the K and k Nan
            #  basically initiliazes the controller paramters to nan
        else:
            traj_distr = prev_traj_distr.copy()

        # Store pol_wt if necessary
        # Algorithm is not badmm
        if type(algorithm) == AlgorithmBADMM:
            pol_wt = algorithm.cur[m].pol_info.pol_wt
            # print(algorithm.cur[m].pol_info,'this is the')
        #  this is like getting the dimension for cutting not the actual slice itself
        idx_x = slice(dX)
        idx_u = slice(dX, dX+dU)

        # Pull out dynamics.
        # basically our A and B
        Fm = traj_info.dynamics.Fm
        fv = traj_info.dynamics.fv

        # Non-SPD correction terms.
        print(self._hyperparams['del0'],'this is del0')
        ...
        while fail:
            ...
            print(fCm,'this is fCm',fcv, 'this is fcv')
