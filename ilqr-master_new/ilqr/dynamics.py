...
'''
AutoDiffDynamics_GPS: Differentiation of dynamics modified for GPS
'''
class AutoDiffDynamics_GPS(Dynamics):

    """Auto-differentiated Dynamics Model."""

    def __init__(self, f, x, state_size=5, action_size=1, i=None, hessians=False, **kwargs):
        """Constructs an AutoDiffDynamics model.

        Args:
            f: Vector Theano tensor expression.
            x: Theano state and action input variables.
            i: Theano tensor time step variable.
            hessians: Evaluate the dynamic model's second order derivatives.
                Default: only use first order derivatives. (i.e. iLQR instead
                of DDP).
        ...
        self._f_linear_xu = as_function(
            self._J, inputs, name="f_linear_diff", **kwargs)

        self._has_hessians = hessians
        if hessians:
            self._Q = hessian_vector(f, non_t_inputs, x_dim)
            self._f_quad_xu = as_function(
                self._Q, inputs, name="f_quad_diff", **kwargs)

        super(AutoDiffDynamics_GPS, self).__init__()
    ...
    def f_linear_diff(self, x, i):
        """Partial derivative of dynamics model with respect to x and u.
    ...
    def f_quad_diff(self, x, i):
        """Second partial derivative of dynamics model with respect to x and u.
        ...
        Returns:
            d^2f/dx^2, d^2f/du^2, d^2f/dudx [state_size, state_size, state_size].
        """
        if not self._has_hessians:
            raise NotImplementedError
